...
export var VSToJS = class {
    ...
    if (begin) {
      try {
        ...
        if (this.isRunOrCode == "Run") {
          document
            .getElementById("console-window")
            .classList.toggle("hidden", false);
          let codeDoc =
            document.getElementById("console").contentWindow.document;
          // console.log("run");
          codeDoc.open();
          codeDoc.writeln(
            `<!DOCTYPE html>\n
        ...
        codeDoc.writeln(
          `<!DOCTYPE html>\n
                    ...
                    ${
                      err.name === "RangeError"
                        ? "CyclicDependence : Irresolvable Cycle(s) Exists"
                        : `UnknownException: Improve The Editor By Opening Issue On GitHub(Just Attach The Exported Graph)`
                    }
                    </code>
                    </body>
                    </html>
  ...
  getExecOut(node) {
    ...
    for (let aNode of node.customClass.execOutPins) {
      if (aNode.wire) X.push(aNode.wire.attrs.dest.getParent());
      else X.push(null);
    }
    // console.log(X);
    return X;
  ...
  getInputPins(node) {
    ...
    for (let aNode of node.customClass.inputPins) {
      if (aNode.wire) {
        X.push({
          node: aNode.wire.attrs.src.getParent(),
          isWire: true,
          srcOutputPinNumber: this.getSrcOutputPinNumber(
            aNode.wire.attrs.src.getParent(),
            aNode.wire
          ),
        });
      } else {
        ...
        X.push({
          node: aNode.textBox.textBox.text(),
          isWire: false,
          srcOutputPinNumber: null,
        });
      }
    }
    return X;
  ...
  coreAlgorithm(node) {
    ...
    if (node.customClass.type.isGetSet) {
      if (node.customClass.type.typeOfNode.slice(0, 3) == "Set") {
        this.script += `${node.customClass.type.typeOfNode.slice(
          4
        )} = ${this.handleInputs(inputPins[0])};\n`;
      ...
      switch (node.customClass.type.typeOfNode) {
        ...
        case "Confirm":
          {
            ...
            this.script += `let _confirm_answer${
              node._id
            } = _confirm(${this.handleInputs(inputPins[0])});\n
                    `;
            this.coreAlgorithm(execOutPins[0]);
          }
          break;
        case "Prompt":
          {
            ...
            this.script += `let [_prompt_ok${node._id}, _prompt_value${
              node._id
            }] = _prompt(${this.handleInputs(
              inputPins[0]
            )}, ${this.handleInputs(inputPins[1])});\n
                        `;
            this.coreAlgorithm(execOutPins[0]);
          }
          break;
        case "OpenWindow":
          {
            ...
            this.script += `let _window_opened${
              node._id
            } = _newWindow(${this.handleInputs(inputPins[0])});\n
                        `;
            this.coreAlgorithm(execOutPins[0]);
          }
          break;
        ...
        case "ForLoop":
          {
            ...
            this.script += `for(let ${forVar} = (${this.handleInputs(
              inputPins[0]
            )}); ${forVar} < (${this.handleInputs(
              inputPins[1]
            )}); ${forVar} += (${this.handleInputs(inputPins[2])})){\n`;
            this.coreAlgorithm(execOutPins[0]);
            this.script += `}\n`;
            this.coreAlgorithm(execOutPins[1]);
          }
        ...
        case "ForEachLoop":
          {
            let forVar = `i${node._id}`; //variable used inside the for loop
            this.script += `${this.handleInputs(
              inputPins[0]
            )}.forEach((value${forVar}, ${forVar}, array${forVar}) => {\n`;
            this.coreAlgorithm(execOutPins[0]);
            this.script += `});\n`;
            this.coreAlgorithm(execOutPins[1]);
          }
        ...
        case "SetByPos":
          {
            this.script += `${this.handleInputs(
              inputPins[2]
            )}[${this.handleInputs(inputPins[0])}] = ${this.handleInputs(
              inputPins[1]
            )};\n`;
            this.coreAlgorithm(execOutPins[0]);
          }
        ...
        case "PushBack":
          {
            this.script += `${this.handleInputs(
              inputPins[1]
            )}.push(${this.handleInputs(inputPins[0])});\n`;
            this.coreAlgorithm(execOutPins[0]);
          }
          break;
        case "PushFront":
          {
            this.script += `${this.handleInputs(
              inputPins[1]
            )}.unshift(${this.handleInputs(inputPins[0])});\n`;
            this.coreAlgorithm(execOutPins[0]);
          }
          break;
        case "PopBack":
        ...
        case "Insert":
          {
            this.script += `${this.handleInputs(
              inputPins[2]
            )}.splice(${this.handleInputs(
              inputPins[0]
            )}, 0, ${this.handleInputs(inputPins[1])});\n`;
            this.coreAlgorithm(execOutPins[0]);
          }
          break;
        case "Reverse":
        ...
        case "Swap":
          {
            this.script += `
                    [${this.handleInputs(inputPins[0])}, ${this.handleInputs(
              inputPins[1]
            )}] = [${this.handleInputs(inputPins[1])}, ${this.handleInputs(
              inputPins[0]
        ...
        case "Sort(Num)":
          {
            this.script += `
              ...
              inputPins[0]
            )}.sort((a, b) => a-b) : ${this.handleInputs(
              inputPins[0]
            )}.sort((a, b) => b-a);\n
                    `;
            this.coreAlgorithm(execOutPins[0]);
        ...
        case "DownloadVideo":
          {
            ...
            this.script += `_downloadVideo(${this.handleInputs(inputPins[0])})`;
            this.coreAlgorithm(execOutPins[0]);
          }
          break;
        case "StrToArray":
          {
            this.script += `let strArray${node._id} = ${this.handleInputs(
              inputPins[0]
            )}.split('');
                    `;
            this.coreAlgorithm(execOutPins[0]);
        ...
        case "ArrayToStr":
          {
            this.script += `let arrayStr${node._id} = ${this.handleInputs(
              inputPins[0]
            )}.join('');
                        `;
            this.coreAlgorithm(execOutPins[0]);
  ...
  handleInputs(inputNode) {
    ...
    switch (inputNode.node.customClass.type.typeOfNode) {
      case "Add":
        {
          expr = `(${this.handleInputs(inputPins[0])} + ${this.handleInputs(
            inputPins[1]
          )})`;
        }
        break;
      ...
      case "Power":
        {
          expr = `Math.pow(${this.handleInputs(
            inputPins[0]
          )}, ${this.handleInputs(inputPins[1])})`;
        }
        break;
      ...
      case "RandomInt":
        {
          expr = `Math.floor(${this.handleInputs(
            inputPins[0]
          )} + Math.random() * (${this.handleInputs(
            inputPins[1]
          )} - ${this.handleInputs(inputPins[0])}))`;
        }
        break;
      ...
      case "Swap":
        {
          expr = `${this.handleInputs(
            inputPins[inputNode.srcOutputPinNumber]
          )}`;
        }
        break;
      case "ForLoop":
      ...
      case "ForEachLoop":
        {
          ...
          if (inputNode.srcOutputPinNumber == 0)
            ...
            expr = `i${inputNode.node._id}`;
          else expr = `arrayi${inputNode.node._id}`;
        }
        break;
      case "Sort(Num)":
        {
      ...
      case "Max(Num)":
        {
          expr = `Math.max(${this.handleInputs(
            inputPins[0]
          )}, ${this.handleInputs(inputPins[1])})`;
        }
        break;
      case "Min(Num)":
        {
          expr = `Math.min(${this.handleInputs(
            inputPins[0]
          )}, ${this.handleInputs(inputPins[1])})`;
        }
        break;
      case "Search":
        {
          ...
          if (inputNode.srcOutputPinNumber == 0) {
            expr = `(${this.handleInputs(
              inputPins[1]
            )}.find((value) => value === ${this.handleInputs(
              inputPins[0]
            )}) === ${this.handleInputs(inputPins[0])})`;
          } else {
            expr = `(${this.handleInputs(
              inputPins[1]
            )}.findIndex((value) => value === ${this.handleInputs(
              inputPins[0]
            )}))`;
          }
        }
      ...
      case "BinarySearch(Num)":
        {
          ...
          if (inputNode.srcOutputPinNumber == 0) {
            this.builtin_functions = {
              ...this.builtin_functions,
              binary_search_exist: true,
            };
            expr = `binary_search_exist(${this.handleInputs(
              inputPins[1]
            )}, ${this.handleInputs(inputPins[0])})`;
          } else if (inputNode.srcOutputPinNumber == 1) {
            this.builtin_functions = {
            ...
            expr = `lower_bound(${this.handleInputs(
              inputPins[1]
            )}, ${this.handleInputs(inputPins[0])})`;
          } else {
            this.builtin_functions = {
            ...
            expr = `upper_bound(${this.handleInputs(
              inputPins[1]
            )}, ${this.handleInputs(inputPins[0])})`;
          }
        }
      ...
      case "GetByPos":
        {
          ...
          expr = `${this.handleInputs(inputPins[1])}[${this.handleInputs(
            inputPins[0]
          )}]`;
        }
        break;
      case "SetByPos":
        {
          expr = `${this.handleInputs(inputPins[2])}[${this.handleInputs(
            inputPins[0]
          )}]`;
        }
        break;
